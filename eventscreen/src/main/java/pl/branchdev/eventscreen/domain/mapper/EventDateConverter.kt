...
object EventDateConverter {
    fun convertDateToDateInformation(currentDate: Date, convertDate: Date): String {
        return when {
            isSameDay(convertDate, currentDate) -> createTodayDateString(convertDate)
            isOneDayBefore(convertDate, currentDate) -> createYesterdayDateString(convertDate)
            else -> formatDateToString(convertDate, EventDtoToEventMapper.MONTH_DAY_YEAR_FORMAT)
        }
    }

    private fun createTodayDateString(date: Date) =
        "Today, ${formatDateToString(date, EventDtoToEventMapper.HOUR_MINUTES_FORMAT)}"

    private fun createYesterdayDateString(date: Date) =
        "Yesterday, ${formatDateToString(date, EventDtoToEventMapper.HOUR_MINUTES_FORMAT)}"
